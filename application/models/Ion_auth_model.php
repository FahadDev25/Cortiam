...
class Ion_auth_model extends CI_Model
{
    ...
    public function register($identity, $password, $email, $additional_data = [])
    {
        ...
        $user_email_text .= '<a href="'.base_url('verify/email').'/' . $additional_data['email_verification_token']. '" style="border-radius:5px;padding:10px 30px;background:#f17221;color:#ffffff;text-align:center;font-weight:bold;text-transform:uppercase;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Verify Email</a></p>';
        $this->mailer->regular_email('Verify Your Email Cortiam!', $user_email_text, $email);

        $user_email_text = '<h3 style="color:#4c525e;">WELCOME TO CORTIAM</h3><h4 style="color:#848994;">Thank You for Joining Cortiam!</h4><p>Your registration has been submitted successfully. Your account is activated and ready for use. You can login to your account with the email and password you selected and complete setting up your profile or add your property for listing.</p><p style="text-align:center;"><a href="'.base_url('login').'" style="border-radius:5px;padding:10px 30px;background:#f17221;color:#ffffff;text-align:center;text-transform:uppercase;font-weight:bold;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Login to Cortiam</a></p>';
    ...
    { 
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, user_type, email_verified_at')
            ->where($this->identity_column, $identity)
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($this->verify_password($password, $user->password, $identity)) {
                if ($user->active == 0) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_deleted');
     ...
     * @author    Mathew
     */
    public function dologin($identity)
    {
        $this->trigger_events('pre_login');
        $this->trigger_events('extra_where');
        ...
        if ($query->num_rows() === 1) {
            ...
            log_message("error", "check user type" . $user->id);
            log_message("error", "check user type" . $user->user_type);
    ...
    protected function _get_hash_parameters($identity = NULL)
    {
        ...
        switch ($this->hash_method) {
            case 'bcrypt':
                $params = [
                    'cost' => $this->config->item('bcrypt_admin_cost', 'ion_auth')
                ];
                break;

            case 'argon2':
                $params = $this->config->item('argon2_admin_params', 'ion_auth');
                break;
    ...
    {
        $query = $this->db->select('email_verification_token')
