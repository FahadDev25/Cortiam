				...
				<div class="d-xl-flex align-items-md-start" id="changeforlowres">
	                  	...
	                  	<?php if($agent['instagram']){?><a href="<?php echo $agent['instagram'];?>" class="btn btn-outline bg-white text-white border-white border-2 btn-icon rounded-round btn-file btn-sm" data-popup="tooltip" target="_blank" title="Instagram" data-container="top"><i class="icon-instagram"></i></a><?php }?>
								...
								<div class="card-body p-0">
									<ul class="nav nav-sidebar mb-2">
										...
										<li class="nav-item">
											<a href="#tdelete" class="nav-link text-danger" data-toggle="tab"><i class="icon-cancel-square"></i> Delete Account</a>
										</li>
									</ul>
								</div>
							</div>
					...
					<div class="tab-content w-100">
						...
						<div class="tab-pane fade" id="tresult">
							<div class="card">
								<div class="card-header bg-transparent header-elements-inline">
									...
									<?php echo (($agent["approval"] == 'Denied')? '<span class="badge badge-pill float-right badge-danger">Denied</span>':(($agent["approval"] == 'Completed')? '<span class="badge badge-pill float-right badge-success">Completed</span>':'<span class="badge badge-pill float-right badge-info">Waiting</span>'));?>
								</div>
								<div class="card-body">
										...
										if ($history) {
											foreach ($history as $history_item) {
												...
												echo '<li class="media media-chat-item-reverse">
													...
													echo '<li class="media">
																	<div class="mr-3">
																		<img src="'.(($history_item['agent_image'])? base_url(str_replace(".jpg","_mini.jpg",$history_item['agent_image'])):base_url('images/userphoto_mini.jpg')).'" class="rounded-circle" width="40" height="40" alt="'.$history_item['agent'].'">
																	</div>
																	<div class="media-body">
																		<div class="media-chat-item">'.nl2br($history_item['message_text']).'</div>
								...
								<?php if (in_array($agent['approval'],array('Waiting','Denied'))) {?>
						...
						<div class="tab-pane fade" id="tdelete">
							...
							<div class="card">
								...
								<div class="card-body">
									<p class="alert alert-warning">You can delete agent account application from within Cortiam. Deleting agent account application is an irreversible process, which we can't revert even if you perform it by accident. Upon deleting agent account application, agent email will be able to be used again to create a future account.</p>
<!--									<p><?php print_r($agent);?></p>-->
									<div>
										<button class="btn btn-danger btn-lg btn-block deleterecordbutton" data-record="<?php echo $agent['agent_id'];?>"><i class="icon-cancel-square"></i> Delete Account Application</button>
									</div>
								</div>
							</div>
