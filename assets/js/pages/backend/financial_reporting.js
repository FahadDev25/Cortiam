jQuery(document).ready(function() {
  ...
  jQuery('#reportrange').daterangepicker({
	...
	jQuery('#reportrange').on('apply.daterangepicker', function(ev, picker) {
		...
		jQuery.ajax({
			type: "post",
			url: cortiamajax.financialdataurl,
		  data: {'startdate' : jQuery('#start_date').val(), 'enddate' : jQuery('#end_date').val()},
			...
			success: function(response){
				if(response.success){
					jQuery('#numbermembership').html('$' + response.reportdata.numbers.membership);
					jQuery('#numberproduct').html('$' + response.reportdata.numbers.product);
					jQuery('#numbercommission').html('$' + response.reportdata.numbers.commission);
					user_zoom.setOption({
				    xAxis: [{
				        data: response.reportdata.formatted.dates
				    }],
					  series: [
					      {data: response.reportdata.formatted.membership},
					      {data: response.reportdata.formatted.product},
					      {data: response.reportdata.formatted.commission},
					  ]
					});
				}else{
					swal.fire({
				    ...
				    "rowId":function(a) {
					  ...
					  "data": response.reportdata.datatable,
						"columnDefs": [
						  ...
						  { "type": "string", "targets": 1, "data": "membership", "title": "Membership Payments", "render": $.fn.dataTable.render.number( ',', '.', 2, '$' )},
						  { "type": "string", "targets": 2, "data": "product", "title": "Package Payments", "render": $.fn.dataTable.render.number( ',', '.', 2, '$' )},
						  { "type": "string", "targets": 3, "data": "commission", "title": "Commission Payments", "render": $.fn.dataTable.render.number( ',', '.', 2, '$' )},
						  { "type": "string", "targets": 4, "data": "total", "title": "Total", "render": $.fn.dataTable.render.number( ',', '.', 2, '$' )},
						],
			      buttons: {
			          buttons: [
			              {
...
user_zoom.setOption({
    ...
    // Add legend
    legend: {
        data: ['Membership Payments', 'Package Payments', 'Commission Payments'],
        itemHeight: 4,
        itemGap: 10
    },
    ...
    series: [
        {
            ...
            data: cortiamajax.reportdata.membership
        },
        {
            ...
            data: cortiamajax.reportdata.product
        },
        {
            ...
            data: cortiamajax.reportdata.commission
        }
    ]
});
