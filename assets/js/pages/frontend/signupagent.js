jQuery(document).ready(function() {
	...
	gtag('event', 'Signup', {'event_category' : 'Visit', 'User Type': 'Agent'});
  ...
  jQuery('#state').select2({
	...
	jQuery('#state').on('select2:select', function (e) {
	  var selected_state = e.params.data;
		...
		if(selected_state.text !== 'Florida')
		{
			...
			jQuery('#response').html('<div class="alert bg-bg-info text-white alert-styled-left alert-dismissible"><button type="button" class="close" data-dismiss="alert"><span>×</span></button><span class="font-weight-semibold">Currently, our service is only available in Florida.</span></div>');
		  ...
		  jQuery('.signupform').block({ message: '<img src="' + cortiamajax.loadingimage + '">', css: {border:'0px',background:'transparent',}, overlayCSS: {backgroundColor:'#fff',opacity:0.7}});
			...
			jQuery(form).ajaxSubmit({
			  ...
			  success: function(i, s, r, a) {
			  	gtag('event', 'Submitted', {'event_category' : 'Signup', 'User Type': 'Agent'});
			  	if(i.askfor){
						swal.fire({
					    ...
					    confirmButtonText: 'Accept',
								...
								jQuery.ajax({
								  ...
								  success: function(i, s, r, a) {
								  	if(i.redirect_to){
									  	...
									  	if(i.success){
									  		gtag('event', 'Waitlist', {'event_category' : 'Added', 'User Type': 'Agent'});
												swal.fire({
							...
							if(e.dismiss == 'cancel'){
								gtag('event', 'Waitlist', {'event_category' : 'Disagree', 'User Type': 'Agent'});
								swal.fire({
			  	...
			  	if(i.redirect_to){
			  		gtag('event', 'Completed', {'event_category' : 'Signup', 'User Type': 'Agent'});
			  		window.location.replace(i.redirect_to);
			  	}
			  	if(i.fail){
	...
	jQuery(document).on('click', '.member-card', function(){

		let plan =	$(this).attr('data-plan');

		//$('.member-card').text("Get Started");
		...
		let setTxt = "Pay Now ( $"+price+" )";
		$('#set-price').text(setTxt);
		
		$("#set-price").attr("data-plan", plan);
